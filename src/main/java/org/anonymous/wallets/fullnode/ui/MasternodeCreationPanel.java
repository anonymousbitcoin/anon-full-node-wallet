 ...
 */
public class MasternodeCreationPanel
  ...
  public MasternodeCreationPanel(ANONClientCaller clientCaller,
    ...
    // tempPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
    // tempPanel.add(sendButton = new JButton("Save Info" + "   \u27A4\u27A4\u27A4"));
    ...
    sendButton.addActionListener(e -> {
      try {
        ...
        JOptionPane.showMessageDialog(
            MasternodeCreationPanel.this.getRootPane().getParent(),
    ...
    saveInfoButton.addActionListener(e -> {
      try {
        ...
        JOptionPane.showMessageDialog(
            MasternodeCreationPanel.this.getRootPane().getParent(), message);
    ...
    this.addressBalanceGatheringThread = new DataGatheringThread<>(
        () -> {
          ...
          String[][] data = MasternodeCreationPanel.this.getAddressPositiveBalanceDataFromWallet();
    ...
    ActionListener alBalancesUpdater = e -> {
      try {
        MasternodeCreationPanel.this.updateWalletAddressPositiveBalanceComboBox();
      } catch (Exception ex) {
        Log.error("Unexpected error: ", ex);
        MasternodeCreationPanel.this.errorReporter.reportError(ex);
      }
    };
    ...
    paste.addActionListener(e -> {
      try {
        ...
        if ((address != null) && (address.trim().length() > 0)) {
          MasternodeCreationPanel.this.destinationAddressField.setText(address);
        }
      } catch (Exception ex) {
        Log.error("Unexpected error", ex);
      }
  ...
  private void sendCash()
    ...
    if (bEncryptedWallet) {
      PasswordDialog pd = new PasswordDialog((JFrame) (MasternodeCreationPanel.this.getRootPane().getParent()));
      pd.setVisible(true);
    ...
    operationStatusTimer = new Timer(2000, e -> {
      try {
        ...
        if ((opComplete != null) && opComplete.booleanValue()) {
          ...
          MasternodeCreationPanel.this.reportCompleteOperationToTheUser(
              amount, sourceAddress, destinationAddress);

          // Lock the wallet again
          if (bEncryptedWallet) {
            MasternodeCreationPanel.this.clientCaller.lockWallet();
          }
        ...
        MasternodeCreationPanel.this.repaint();
      } catch (Exception ex) {
        Log.error("Unexpected error: ", ex);
        MasternodeCreationPanel.this.errorReporter.reportError(ex);
      }
